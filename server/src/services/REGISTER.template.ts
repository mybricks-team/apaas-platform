  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const res = await _execSQL(sql, { args: obj });
	    ...
	    await _execSQL(
				`INSERT INTO D_${projectId}_系统用户_VIEW (id, _STATUS_DELETED, 用户名, 电话号码, 邮箱, 密码, weixinOpenid, weixinUnionid)
 							VALUES (${genUniqueId()}, 0, ${getValue('username')}, ${getValue('phone')}, ${getValue('email')}, ${getValue('password')}, ${getValue('weixinOpenid')}, ${getValue('weixinUnionid')});`,
		    { args: obj }
	    );
